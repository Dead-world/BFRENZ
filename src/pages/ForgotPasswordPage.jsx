import React, { useState, useEffect } from 'react'; // 🟢 FIXED: Explicit hook binding
...
if (typeof document !== 'undefined') {
  ...
  styleEl.innerHTML = `
    .recovery-wrapper { background-color: #000000 !important; color: #ffffff !important; min-height: 100vh !important; display: flex !important; align-items: center !important; justify-content: center !important; font-family: 'Courier New', monospace !important; padding: 20px !important; box-sizing: border-box !important; }
      ...
      <div className="recovery-card">
        ...
        <form onSubmit={handleRequestReset}>
          <div className="recovery-form-group">
            ...
            <input type="email" className="recovery-input" required placeholder="ENTER REGISTERED EMAIL" value={email} onChange={(e) => setEmail(e.target.value)} disabled={isSubmitting} />
          </div>
          <button type="submit" className="recovery-submit-btn" disabled={isSubmitting}>
            {isSubmitting ? 'Broadcasting link...' : 'Send Recovery Email'}
          </button>
          {status.text && ( <div className={`recovery-status-alert ${status.type}`}> {status.type === 'success' ? '✔' : '⚠️'} {status.text} </div> )}
        </form>
